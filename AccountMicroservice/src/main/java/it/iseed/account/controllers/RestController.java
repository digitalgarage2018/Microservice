package it.iseed.account.controllers;


import io.jsonwebtoken.ExpiredJwtException;
import it.iseed.account.entities.Operation;
import it.iseed.account.entities.User;
import it.iseed.account.entities.UserRequest;
import it.iseed.account.services.LoginService;
import it.iseed.account.services.LoginServiceImpl;
import it.iseed.account.services.OperationService;
...
public class RestController {
    ...
    @RequestMapping(name = "/newuser1", method = RequestMethod.POST)
    ...
    public ResponseEntity<JsonResponseBody> loginUserJson(@RequestBody UserRequest request){
        //check if user exists in DB -> if exists generate JWT and send back to client
        try {
            Optional<User> userr = loginService.getUserFromDbAndVerifyPassword(request.getId(), request.getPassword());
            if(userr.isPresent()){
                User user = userr.get();
                String jwt = loginService.createJwt(user.getId(), user.getUsername(), user.getPermission(), new Date());
                return ResponseEntity.status(HttpStatus.OK).header("jwt", jwt).body(new JsonResponseBody(HttpStatus.OK.value(), "Success! User logged in!"));
